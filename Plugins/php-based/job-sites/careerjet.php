<?php
const PROJECT_URL="http://github.com/selner/job_scooper_v4";
/**
...
//  $url .= '&user_agent=' . urlencode($_SERVER['HTTP_USER_AGENT']);
    $url .= '&user_agent=' . urlencode($this->version);

    // determine current page
    $current_page_url = PROJECT_URL;
    ...
    public function _getMyJobsForSearchFromJobsAPI_(\JobScooper\DataAccess\UserSearchSiteRun $searchDetails)
    {
        ...
        while (count($arrSearches) > 0) {
            ...
            while ($noMoreJobs !== true) {
                ...
                // Then call the search methods (see below for parameters)
                LogMessage("Calling CareerJet API for search: " . getArrayDebugOutput($search));
...
//                        ## as 'location_id' parameter
                ...
                if ($result->type == 'JOBS') {
                    ...
                    $arrJobs = $this->_parseAPIJobs($result->jobs);

                    $this->saveSearchReturnedJobs($arrJobs, $searchDetails);
                    $nTotalJobs = $nTotalJobs + \count($arrJobs);
                }
                $pageNumber++;
