...
class GeocodeApiLoggedProvider extends AbstractProvider implements LocaleAwareProviderInterface
{
	...
	{
		$query = sprintf(self::ENDPOINT_URL, $this->server, rawurlencode($address), rawurlencode($this->region), rawurlencode($this->apiKey));
	...
	protected function buildQuery($query)
	{
		if (!empty($this->getRegion())) {
			$query = sprintf('%s&countrycode=%s', $query, $this->getRegion());
		}

		if (!empty($this->getApiKey())) {
			$query = sprintf('%s&apikey=%s', $query, $this->apiKey);
		}
