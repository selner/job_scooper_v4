...
class ClassJobsNotifier extends \Jobscooper\BasePlugin\JobsSiteCommon
{
...
//                $name = $nameParts[count($nameParts)-1];
    ...
    function processNotifications()
    {
        ...
        if(isDebug() === true) {
            ...
            //
            // In a debug run, src the full details of the keywords we used for auto-matching
            //
            if(isDebug() == true) {
    ...
    private function _filterAndWriteListToFile_($arrJobsList, $strFilterToApply = null, $fileDetails=null)
    {
    ...
    private function _getFullFileContents_($detailsFile)
    {
        ...
        {
            $GLOBALS['logger']->logLine("Unable to get contents from '". var_export($detailsFile, true) ."' to src in email.  Failing notification.", \Scooper\C__DISPLAY_ERROR__);
            return null;
        }
    ...
    private function _wrapCSSStyleOnHTML_($strHTML)
    {
        ...
        $css = file_get_contents(dirname(dirname(__FILE__)) . '/src/static/CSVTableStyle.css');
