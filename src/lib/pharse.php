...
class Pharse{
    ...
    static function options(array $options){
        ...
        $arg_string = trim(str_replace('--', '-', $arg_string));
        ...
        //
        $nTotalOptions = -1;
//        $arg_string = preg_replace('/\s(-)([[:alnum:][:punct:]]{1,})/', "  $1$2", $arg_string, $nTotalOptions);
        $arg_string = preg_replace('/(^|\s)(-)([[:alnum:][:punct:]]{1,})([\s"[[:alnum:]\s[:punct:]]{1,}")*/', "<<<OPTIONTOKEN>>>$3", $arg_string, -1, $nTotalOptions);

        $args       = explode('<<<OPTIONTOKEN>>>', $arg_string);
        array_shift($args);
