...
abstract class AbstractBaseJobsPlugin extends \Jobscooper\BasePlugin\JobsSiteCommon
{
    ...
    function __construct($strOutputDirectory = null, $attributes = null)
    {
        ...
        if(stristr($this->strBaseURLFormat, "***NUMBER_DAYS***") == false)
            $this->additionalFlags[] = C__JOB_DAYS_VALUE_NOTAPPLICABLE;
...
//                $this->markJobsList_withAutoItems($tmpSearchJobs);
    ...
    protected function getLocationURLValue($searchDetails, $locSettingSets = null)
    {
        ...
        {
            throw new ErrorException($this->siteName . " does not support the ***LOCATION*** replacement value in a BasePlugin URL.  Please review and change your BasePlugin URL format to remove the location value.  Aborting all searches for " . $this->siteName, \Scooper\C__DISPLAY_ERROR__);
        }
    ...
    protected function _getBaseURLFormat_($searchDetails = null, $nPage = null, $nItem = null)
    {
        ...
        {
            throw new ErrorException("Could not find BasePlugin URL format for " . $this->siteName . ".  Aborting all searches for " . $this->siteName, \Scooper\C__DISPLAY_ERROR__);
        }
        return $strBaseURL;
    }
    ...
    private function _getFileStoreKeyForSearch($searchSettings, $prefix = "")
    {
        ...
        if(!$this->isBitFlagSet(C__JOB_DAYS_VALUE_NOTAPPLICABLE))
    ...
    private function _getMyJobsForSearchFromWebpage_(&$searchDetails)
    {
        ...
        try
        {
            ...
            if(!$this->isBitFlagSet(C__JOB_ITEMCOUNT_NOTAPPLICABLE) || !$this->isBitFlagSet(C__JOB_PAGECOUNT_NOTAPPLICABLE))
            {
                ...
                if(is_null($strTotalResults)) throw new InvalidArgumentException("Null value returned for total result count.");
            ...
            else
            {
                ...
                while ($nPageCount <= $totalPagesCount )
                {
                    ...
                    try
                    {
                        ...
                        if(is_null($arrPageJobsList) || !is_array($arrPageJobsList) )
                        {
