...
class ClassConfig extends \Jobscooper\BasePlugin\AbstractBaseJobsPlugin
{
    ...
    {
        $this->initialize();
        parent::__construct($strOutputDirectory, $attributes);
...
//        $this->_setPreviouslyReviewedJobsInputFiles__setPreviouslyReviewedJobsInputFiles_();
    ...
    private function __addInputFile__($iniInputFileItem)
    {
        ...
        elseif(isset($iniInputFileItem['file']))
        {
            $tempFileDetails = \Scooper\parseFilePath($this->arrFileDetails['input_folder']['directory'].$iniInputFileItem['file'], true);
        }
                ...
                foreach($config['search_location_setting_set'] as $iniSettings)
                {
                    if(count($iniSettings) > 1)
                    {
                        ...
                        if(isset($iniSettings['name']))
                        {
                            ...
                            {
                                throw new Exception("Error: Invalid location set data loaded from configs.  Did you inadvertently src the same location set [" . $iniSettings['name'][0] . "] twice?");
                            }
            ...
            foreach($GLOBALS['USERDATA']['configuration_settings']['keyword_sets'] as $keywordSet)
            {
                ...
                if(isset($GLOBALS['USERDATA']['configuration_settings']['included_sites']))
                {
                    ...
                    if(count($GLOBALS['USERDATA']['configuration_settings']['included_sites'])>0)
                    {
                        ...
                        {
                            $keyPrefix = \Scooper\strScrub($siteToSearch, FOR_LOOKUP_VALUE_MATCHING) . \Scooper\strScrub($keywordSet['key'], FOR_LOOKUP_VALUE_MATCHING);
                            ...
                            else // if not, we need to add search for each keyword using that word as a single value in a keyword set
                            {
                                ...
                                foreach($arrKeys as $key)
                                {
                                    ...
                                    $newSearch['key'] = $keyPrefix . "-" .\Scooper\strScrub($key, FOR_LOOKUP_VALUE_MATCHING);
        ...
        if(isset($primaryLocationSet))
        {
            foreach(array_keys($arrSearches) as $searchKey)
            {
                ...
                if(!is_null($locTypeNeeded))
                {
                    ...
                    $GLOBALS['USERDATA']['configuration_settings']['searches'][$searchKey]['key'] = $GLOBALS['USERDATA']['configuration_settings']['searches'][$searchKey]['key'] . "~" . strtolower($primaryLocationSet['key']);
        ...
        if(isset($GLOBALS['USERDATA']['configuration_settings']['location_sets']) && is_array($GLOBALS['USERDATA']['configuration_settings']['location_sets']) && count($GLOBALS['USERDATA']['configuration_settings']['location_sets']) > 1)
        {
            ...
            foreach(array_keys($arrPossibleSearches_Start) as $l)
            {
                ...
                foreach($GLOBALS['USERDATA']['configuration_settings']['location_sets'] as $locSet)
                {
                    ...
                    if(substr_count($strOldSearchKey, "~") > 0) { $strOldSearchKey = explode("~", $strOldSearchKey)[0]; }
                    $newSearch['key'] = $strOldSearchKey . "~" . strtolower($locSet['key']);
